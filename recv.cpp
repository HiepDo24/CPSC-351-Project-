...
string recvFileName()
{
	...
	if (msgrcv(msqid, &fmsg, sizeof(fileNameMsg) - sizeof(long), FILE_NAME_TRANSFER_TYPE, 0) ==-1)
	{
		perror("msgsnd");
		exit(1);
	}
...
void init(int& shmid, int& msqid, void*& sharedMemPtr)
{
	...
	*/
	...
	msqid = msgget(key, S_IRUSE | S_IWUSR | IPC_CREAT);
	if (msqid == -1) {
		perror("msgget");
	}
...
unsigned long mainLoop(const char* fileName)
{
	...
	{	
		...
		if(msgSize != 0)
		{
			...
			numBytesRecv = msgSize + numBytesRecv;
			 ...
			 * of type ackMessage with mtype field set to RECV_DONE_TYPE. 
 			 */
			ackMessage rts_msg;
			rts_msg.mtype = RECV_DONE_TYPE;
			if (msgsnd(msqid, &rts_msg, sizeof(ackMessage) - sizeof(long), 0) ==-1)
			{
				perror("msgsnd");
				exit(1);
			}
